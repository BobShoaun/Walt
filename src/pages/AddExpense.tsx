  ...
  return (
    <main className="p-2 h-full">
      ...
      >
        ...
        <div>
          ...
          >
            {categories.map((category) => (
              <option key={category.slug}>{category.image} {category.name}</option>
            ))}
          </select>
        </div>
        ...
        <button
          type="submit"
          className="w-full bg-indigo-200 text-indigo-800 py-2 border border-indigo-700 cursor-pointer"
        >
          Add Expense
        </button>
      </form>
